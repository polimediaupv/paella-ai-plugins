...
import "./AIToolChat.css";
import { usePaellaPlugin } from './react-context.js';
...
export default function AIToolChat({ className = "" }) {
    const paellaPlugin = usePaellaPlugin();
    const [showWelcomeView, setShowWelcomeView] = useState(paellaPlugin.showWelcomeMessage);
    const [showSettingsView, setShowSettingsView] = useState(false);
    const [settings, setSettings] = useState(paellaPlugin?.webllmSettings || {});
    const [chats, setChats] = useState(paellaPlugin.loadChats())
    ...
    useEffect(() => {
        if (currentChatIndex && !processing) {
    ...
    const handleDeleteChat = (i) => {
        const newChats = chats.filter((chat, j) => i !== j);
        setChats(newChats);
    ...
    const submitMessage = async (e) => {
        ...
        if (paellaPlugin.model == null) {
            await paellaPlugin.loadModel(settings, (progress) => {          
                messageResponse.text = progress.text;
                setChats([...newChats]);
            });
        }
...
`
        ...
        for await (const chunk of stream) {
            messageResponse.text += chunk.content
            setChats([...newChats]);
        }
