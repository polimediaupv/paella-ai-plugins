...
export default function ChatSettings({
    ...
    const [modelType, setModelType] = useState(settings?.modelType ?? 'webllm');
    const [openAIURL, setOpenAIURL] = useState(settings?.openAIURL ?? 'https://api.openai.com/v1');
    const [openAIPasswd, setOpenAIPasswd] = useState(settings?.openAIPasswd ?? '');
    const [model, setModel] = useState(settings?.model ?? (modelType == 'webllm' ? 'Llama-3.1-8B-Instruct-q4f32_1-MLC' : 'gpt-4o'));
    const [contextWindowLength, setContextWindowLength] = useState(settings?.contextWindowLength ?? '1K');
    ...
    const [frequecyPenalty, setFrequecyPenalty] = useState(settings?.frequecyPenalty ?? 0.0);
    ...
    return (        
        <div className="settings">            
            ...
            <li>
                    ...
                    <select defaultValue={modelType} onChange={(e) => setModelType(e.target.value)}>
            ...
            <ul className="settings-list">
                <li>
                    ...
                    <textarea type="text" aria-describedby={paellaPlugin.player.translate("System prompt")} placeholder="You are a helpful assistant" rows={3} cols={30} value={systemPrompt} onChange={(e) => setSystemPrompt(e.target.value)} />
                </li>
            </ul>
